      ...
      try
      {
        ...
        while(iter.hasNext())
        {
          ...
          if(is_active(active_string))
          {
            ...
            {
              event_collector.dispatch("SYSTEM%%" + tags + "%%Too few instances of " + mtch + " running");// (" + ps_hash.get(mtch)[4] + " should be " + ps_hash.get(mtch)[2] + ")");
            ...
            {
              event_collector.dispatch("SYSTEM%%" + tags + "%%Too many instances of " + mtch + " running");// (" + ps_hash.get(mtch)[4] + " should be " + ps_hash.get(mtch)[3] + ")");
              log("Process: " + mtch + " " + ps_hash.get(mtch)[4] + " / " + ps_hash.get(mtch)[2] + " -> " + ps_hash.get(mtch)[3]);
            }
          }
